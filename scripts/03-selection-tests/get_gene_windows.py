...
infile = "../data/mm10-cds-window-overlap-sorted.bed";
#windowfile = "../../windows/data/10kb-0.5-0.5-windows-filter.tsv";
#windows_indir = "../../windows/tree/";
#concat_tree_file = "../tree/concat/concat.cf.rooted.tree"
#gene_tree_dir = "../tree/loci/";

#chrome_tree_outdir = "../tree/penn-7spec/chrome/";
#window_tree_outdir = "../tree/penn-7spec/window/";

outfilename = "../data/mm10-cds-windows.csv";
...
with open(outfilename, "w") as outfile:
    ...
    core.PWS("# " + core.getDateTime() + " Reading concatenated tree:" + concat_gene_tree_file, outfile);
    concat_tree = open(concat_gene_tree_file, "r").read().strip();
    ...
    for chrome in chrome_sizes:
        ...
        for treetype in ["", "-rooted"]:
            chrome_treefile = chrome + "_10kb_iqtree" + treetype + ".treefile"
            chrome_tree_infile = os.path.join(windows_indir, chrome + "-iqtree", "concat", chrome_treefile);
            chrome_tree = open(chrome_tree_infile, "r").read().strip();
    ...
    with open(windowfile) as csvfile:
        ...
        for line in windows_reader:
            ...
            if first:
                headers = line + ["Gene ID", "Transcript ID", "CDS start", "CDS end", "Overlap", "Gene tree", "Gene tree match window tree", "Gene tree match chrome tree", "Gene tree match concat tree"];
                first = False;
                continue;
    ...
    for line in open(infile):
        ...
        window, tstart, tend, ids, overlap, chrome = line[9], line[1], line[2], line[3], line[10], line[0];
        gid, tid = ids.split(";");
        # Read the transcripts and overlaps with windows.

        gene_tree_file = os.path.join(gene_tree_dir, gid + "-penn_7spec_cds.NT.trim.NT.macse.trim.filter", gid + "-penn_7spec_cds.NT.trim.NT.macse.trim.filter-rooted.treefile");
        if not os.path.isfile(gene_tree_file) or chrome == "chrY":
            genes_skipped += 1;
            continue;
        ...
        for treetype in ["", "-rooted"]:
            window_str = chrome + "-" + window.replace(":", "-") + "-mafft-trimal";
            window_treefile_in = window_str + treetype + ".treefile"
            window_tree_infile = os.path.join(windows_indir, chrome + "-iqtree", "loci", window_str, window_treefile_in);

            window_tree = open(chrome_tree_infile, "r").read().strip();
        ...
        outline = windows[window] + [gid, tid, tstart, tend, overlap, '"' + gene_tree + '"', gt_wt_match, gt_ct_match, gt_cat_match];
        # Get the output
