...
tree_file = paste(datadir, "05_penn_7spec_iqtree.cf.rooted.tree", sep="")
...
concat_mg_local_file = paste(datadir, "hyphy/penn-7spec-mg94-local-concat.csv", sep="")
gt_mg_local_file = paste(datadir, "hyphy/penn-7spec-mg94-local-gt.csv", sep="")

concat_m1a_file = paste(datadir, "paml/penn-7spec-m1a-concat.csv", sep="")
concat_m2a_file = paste(datadir, "paml/penn-7spec-m2a-concat.csv", sep="")
gt_m1a_file = paste(datadir, "paml/penn-7spec-m1a-gt.csv", sep="")
gt_m2a_file = paste(datadir, "paml/penn-7spec-m2a-gt.csv", sep="")

concat_absrel_file = paste(datadir, "hyphy/penn-7spec-absrel-concat-bonf.csv", sep="")
gt_absrel_file = paste(datadir, "hyphy/penn-7spec-absrel-gt-bonf.csv", sep="")

concat_busted_file = paste(datadir, "hyphy/penn-7spec-busted-concat.csv", sep="")
gt_busted_file = paste(datadir, "hyphy/penn-7spec-busted-gt.csv", sep="")
...
fig_6 = plot_grid(fig_6a, fig_6bc_leg, nrow=2, labels=c("A", ""), label_size=16, rel_heights=c(1,0.8))
print(fig_6)

######################

if(save_fig){
  fig6file = "../figs/fig6.png"
  cat(as.character(Sys.time()), " | Fig6: Saving figure:", fig6file, "\n")
  ggsave(filename=fig6file, fig_6, width=6, height=8, units="in")
}
